...
VoiceRecorder.defaultSettings = {
  swf: "",
  flashvars: {
      apiserver: "https://web.archive.org/web/20120118212735/http://goanimate.com/",
      appCode: "go",
      u_info: ""
  }
};
...
var GoLite = (function(E) {
  ...
  function G() {
      ...
      E.each(A, function(Z, a) {
          var Y = {};
          var b = a.data("cid").replace(".", ":");
          Y[b] = Z + 1;
          X.characters.push(Y)
      });
      X.script = GoLite.getScripts();
      if (J) {
          X.opening_closing.opening_characters = {};
          X.opening_closing.opening_characters.facial = E.extend({}, X.script[0].facial);
          for (x in X.opening_closing.opening_characters.facial) {
              X.opening_closing.opening_characters.facial[x] = "default"
          }
          X.opening_closing.closing_characters = {};
          X.opening_closing.closing_characters.facial = E.extend({}, X.script[(X.script.length - 1)].facial)
      }
      return X
  }
  function I(Y, X) {
  ...
  function O() {
      var X = {
          M: ["eric", "oc_paul", "joey", "oc_dave", "oc_steven"],
          F: ["oc_kate", "oc_julie", "kimberly", "oc_allison", "jennifer", "kendra", "oc_susan"]
      };
  ...
  return {
      ...
      preview: function() {
          ...
          P = true;
          E.post("/ajax/previewText2Video", X, function(Y) {
              P = false;
              if (Y.error) {
...
(function(A) {
  ...
  ;
  B.prototype.disableItem = function(C) {
      C.addClass("on");
      C.draggable("disable")
  }
  ;
  B.prototype.enableItem = function(C) {
      C.removeClass("on");
      C.draggable("enable")
  }
  ;
  B.prototype.getItemByCid = function(D) {
  ...
  ;
  B.prototype.setHost = function(D) {
      if (this.host) {
  ...
  ;
  B.prototype.addItem = function(D) {
  ...
  ;
  B.prototype.replaceItem = function(C, E) {
      E.addClass("on");
      E.draggable("disable");
      var D = C.data("item");
      D.removeClass("on");
  ...
  ;
  B.prototype.removeItem = function(C, E) {
  ...
  ;
  B.prototype.confirmRemoveItem = function(C) {
      return true
  }
  ;
