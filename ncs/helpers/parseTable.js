...
module.exports = table => {
    ...
    const songs = Array.from(rows).map((row) => {
        ...
        const artists = artistsEl ? artistsEl.split(', ').map((artist) => {
        ...
        const tags = Array.from(tags_els).map((el) => {
        ...
        return {
            ...
            tags,
            download: {
                regular: availableTypes.includes('regular') ? `https://ncs.io/track/download/${songId}` : undefined,
                instrumental: availableTypes.includes('instrumental') ? `https://ncs.io/track/download/i_${songId}` : undefined
            }
        };
    });
    return songs;
