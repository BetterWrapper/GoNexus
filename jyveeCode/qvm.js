		...
		case "/ajax/previewText2Video": {
			loadPost(req, res).then(f => {
				...
				async function gentts() {
					let i = ouri;
					if (mcids[i] == "") try {
						if (!voicez[i]) return res.end(JSON.stringify({
							error: `Please select a voice for dialog ${i + 1}.`
						}))
						if (!texts[i]) return res.end(JSON.stringify({
							error: `Please enter some text on dialog ${i + 1}.`
						}))
						console.log("page:", i);
						(async () => {
							try {
